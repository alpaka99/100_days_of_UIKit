//
//  ResultViewControllerTableViewController.swift
//  UIKit_day_91
//
...
class ResultViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        let query = CKQuery(recordType: "Suggestion", predicate: pred)
        query.sortDescriptors = [sort]
        
        CloudStore.store.container.publicCloudDatabase.perform(query, inZoneWith: nil) { [unowned self] results, error in
            if let error = error {
    ...
    @objc func downloadTapped() {
        ...
        CloudStore.store.container.publicCloudDatabase.fetch(withRecordID: whistle.recordID) { [unowned self] record, error in
            if let error = error {
                DispatchQueue.main.async {
                    self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Download", style: .plain, target: self, action: #selector(downloadTapped))
                }
            } else {
    ...
    @objc func listenTapped() {
        do {
            ...
            let ac = UIAlertController(title: "Playback failed", message: "There was a problem playing whistle", preferredStyle: .alert)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if whistle.comments.count == 0 {
                cell.textLabel?.text = "Comment: None"
            } else {
        ...
        CloudStore.store.container.publicCloudDatabase.save(whistleRecord) { [unowned self] (record, error) in
            DispatchQueue.main.async {
                if error == nil {
                    ...
                    let ac = UIAlertController(title: "There was errorr uploading suggestion", message: "Error: \(String(describing: error?.localizedDescription))", preferredStyle: .alert)
                    ac.addAction(UIAlertAction(title: "OK", style: .default))
